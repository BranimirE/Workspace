...
public:
  ...
  bool endWith(string cad, string end){
    if(cad.size() < end.size())
      return false;
    for(int i = end.size() - 1, j = cad.size() - 1; i >= 0; i--, j--)
      if(end[i] != cad[j])
         return false;
    return true;
  }
  double find(vector <string> goodSuffixes) {
    ...
    for(int i = 0; i < n; i++)
      if(sw[i] == false){
        ans += pow(10.0, -(int)goodSuffixes[i].size());
        for(int j = i + 1; j < n; j++)
          if(endWith(goodSuffixes[j], goodSuffixes[i]))
             sw[j] = true;
      }
    return ans;      
  }
