...
public:
  string isAble(long long D, int T, int B) {
    long long int dist = D, jumps = T, salt = B, ans = 0;
    long long int big = dist/salt;
    long long int small = dist - (big * salt);
    ans = big + small;
    if(ans > jumps || dist < jumps) 
      return "Impossible";
    return ((dist-jumps)%(salt-1) == 0)?"Possible":"Impossible";
  }
};
